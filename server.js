...
app.use(session({
  secret: process.env.SESSION_SECRET || 'runsheet-dev-secret',
  resave: false,
...
// Root — redirect to app or connect
app.get('/', (req, res) => {
  if (req.session.tokenSet) return res.redirect('/app');
...
body{font-family:'DM Sans',sans-serif;background:#0F1F12;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}
.card{background:white;border-radius:16px;padding:48px 40px;max-width:420px;width:100%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.3)}
.logo{font-family:'DM Serif Display',serif;font-size:32px;color:#0F1F12;margin-bottom:4px}
.logo span{color:#52B788}
.tagline{font-size:12px;color:#718096;text-transform:uppercase;letter-spacing:0.12em;margin-bottom:32px}
h2{font-size:20px;color:#0F1F12;margin-bottom:12px}
p{font-size:14px;color:#5A6672;line-height:1.6;margin-bottom:28px}
.btn-xero{display:block;background:#0F1F12;color:white;text-decoration:none;padding:14px 28px;border-radius:10px;font-size:15px;font-weight:700;transition:background 0.2s;margin-bottom:12px}
.btn-xero:hover{background:#1A3320}
.btn-demo{display:block;background:transparent;color:#5A6672;text-decoration:none;padding:12px 28px;border-radius:10px;font-size:14px;border:1.5px solid #D4E6DA;transition:all 0.2s}
.btn-demo:hover{border-color:#2D6A4F;color:#2D6A4F}
.security{font-size:11px;color:#A0ADB8;margin-top:20px}
</style>
</head>
<body>
<div class="card">
  ...
  <p>Runsheet reads your invoices, bills, and payroll from Xero to build your 52-week cashflow forecast. Read-only access — we never modify your data.</p>
  <a class="btn-xero" href="/auth">Connect with Xero →</a>
  <a class="btn-demo" href="/app?demo=true">Try with demo data</a>
  <p class="security">🔒 Secure OAuth 2.0 · Read-only · Disconnect anytime</p>
</div>
</body>
</html>`);
});
...
// Tenant selection (if multiple Xero orgs)
app.get('/select-org', requireAuth, (req, res) => {
  ...
  res.send(`<!DOCTYPE html><html><head><title>Select Organisation</title>
...
.card{background:white;border-radius:12px;padding:32px;max-width:480px;width:100%}
h2{font-size:20px;color:#0F1F12;margin-bottom:8px}p{color:#5A6672;margin-bottom:24px;font-size:14px}
...
<body><div class="card"><h2>Select Organisation</h2><p>Choose which Xero organisation to connect to Runsheet:</p>
${tenants.map(t => `<a class="org" href="/select-org/${t.tenantId}">${t.tenantName}</a>`).join('')}
</div></body></html>`);
});

app.get('/select-org/:tenantId', requireAuth, (req, res) => {
  const tenant = (req.session.tenants || []).find(t => t.tenantId === req.params.tenantId);
...
app.get('/api/invoices', requireAuth, async (req, res) => {
  try {
    ...
    const response = await xero.accountingApi.getInvoices(
      ...
      undefined, undefined, ['AUTHORISED', 'SUBMITTED'],
    ...
    const invoices = (response.body.invoices || []).map(inv => ({
      ...
      status: inv.status,
      issued: inv.date?.toISOString?.()?.substring(0,10) || ''
    }));
    res.json({ invoices });
  } catch (err) {
    console.error('Invoices error:', err?.body || err.message);
...
app.get('/api/bills', requireAuth, async (req, res) => {
  try {
    ...
    const response = await xero.accountingApi.getInvoices(
      tenantId, undefined, undefined, undefined, undefined,
      undefined, ['ACCPAY'], ['AUTHORISED', 'SUBMITTED'],
      undefined, undefined, undefined, undefined, true
    );
    const bills = (response.body.invoices || []).map(b => ({
      id: b.invoiceID,
      ref: b.invoiceNumber,
      supplier: b.contact?.name || 'Unknown',
      amount: b.total,
      amountDue: b.amountDue,
      due: b.dueDate?.toISOString?.()?.substring(0,10) || '',
      status: b.status,
      lineItems: (b.lineItems || []).map(li => li.description)
    }));
    res.json({ bills });
  } catch (err) {
    console.error('Bills error:', err?.body || err.message);
...
app.get('/api/bank-balance', requireAuth, async (req, res) => {
  try {
    ...
    const accounts = (response.body.accounts || []).map(a => ({
      id: a.accountID,
      name: a.name,
      code: a.code,
      type: a.type,
      balance: a.reportingCodeUpdated || 0
    }));
    res.json({ accounts });
  } catch (err) {
    console.error('Bank error:', err?.body || err.message);
    res.status(500).json({ error: 'Failed to fetch bank accounts', detail: err.message });
  }
});
...
app.get('/api/payroll', requireAuth, async (req, res) => {
  try {
    ...
    const payRuns = (response.body.payRuns || []).slice(0, 12).map(pr => ({
      id: pr.payRunID,
      period: pr.payRunPeriod,
      startDate: pr.startDate?.toISOString?.()?.substring(0,10),
      endDate: pr.endDate?.toISOString?.()?.substring(0,10),
      paymentDate: pr.paymentDate?.toISOString?.()?.substring(0,10),
      totalNetPay: pr.totalNetPay,
      totalEmployerTax: pr.totalEmployerTax,
      totalEmployeeTax: pr.totalEmployeeTax,
      wages: pr.totalGross,
      super: pr.totalSuperannuation,
      payg: pr.totalPAYG
    }));
    res.json({ payRuns });
  } catch (err) {
    console.error('Payroll error:', err?.body || err.message);
    // Payroll API may not be available on all Xero plans
    res.json({ payRuns: [], note: 'Payroll data requires Xero Payroll subscription' });
  }
});
...
app.get('/api/organisation', requireAuth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      abn: org?.taxNumber,
      baseDueDate: org?.salesTaxPeriod,
      financialYearEnd: org?.financialYearEndMonth,
      currency: org?.baseCurrency
    });
  } catch (err) {
    res.status(500).json({ error: 'Failed to fetch organisation', detail: err.message });
  }
});
...
app.get('/api/summary', requireAuth, async (req, res) => {
  try {
    ...
    // Parallel fetch
    const [invoicesResp, billsResp] = await Promise.all([
      xero.accountingApi.getInvoices(tenantId, undefined, undefined, undefined, undefined,
        undefined, undefined, ['AUTHORISED'], undefined, undefined, undefined, undefined, true),
      xero.accountingApi.getInvoices(tenantId, undefined, undefined, undefined, undefined,
        undefined, ['ACCPAY'], ['AUTHORISED'], undefined, undefined, undefined, undefined, true)
    ]);

    const invoices = (invoicesResp.body.invoices || [])
      .filter(i => i.type === 'ACCREC' && i.amountDue > 0)
      .map(i => ({
        client: i.contact?.name,
        ref: i.invoiceNumber,
        amount: i.amountDue,
        due: i.dueDate?.toISOString?.()?.substring(0,10),
        status: i.status
      }));

    const bills = (billsResp.body.invoices || [])
    ...
    res.json({
      tenantName: req.session.activeTenantName,
      invoices,
      bills,
      totalReceivables,
      totalPayables,
    ...
    console.error('Summary error:', err?.body || err.message);
...
app.get('/app', async (req, res) => {
  ...
  const orgName = req.session.activeTenantName || 'Demo Company';
  const isMultiTenant = (req.session.tenants || []).length > 1;

  // Serve the full app HTML
...
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'DM Sans',sans-serif;background:var(--sand);color:var(--text);min-height:100vh;font-size:14px}
...
.org-name{font-size:12px;color:rgba(255,255,255,0.5)}
.topbar-right{display:flex;align-items:center;gap:14px}
${isDemo?'.demo-badge{background:var(--orange);color:#fff;font-size:10px;font-weight:700;padding:3px 8px;border-radius:10px;text-transform:uppercase}':''}
.btn-disconnect{background:transparent;border:1px solid rgba(255,255,255,0.2);color:rgba(255,255,255,0.6);padding:5px 12px;border-radius:6px;font-size:12px;cursor:pointer;font-family:"DM Sans",sans-serif}
...
.nav-btn:hover{background:var(--sand);color:var(--text)}
.nav-btn.active{background:var(--pale);color:var(--dark);font-weight:700}
...
.stat-val{font-family:'DM Serif Display',serif;font-size:26px;color:var(--text)}
...
.badge{display:inline-flex;align-items:center;padding:2px 9px;border-radius:10px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.04em;white-space:nowrap}
...
.spinner{display:inline-block;width:20px;height:20px;border:2px solid var(--border);border-top:2px solid var(--accent);border-radius:50%;animation:spin 0.8s linear infinite;margin-bottom:10px}
...
.wk-in{width:100px;color:#2D6A4F;font-weight:600;text-align:right;flex-shrink:0}
...
.wk-bal.ok{color:#2D6A4F}.wk-bal.low{color:var(--amber)}.wk-bal.neg{color:var(--danger)}
...
.danger-row{background:rgba(192,57,43,0.06);border-radius:4px}
.amber-row{background:rgba(183,105,10,0.05);border-radius:4px}
...
.form-field input,.form-field select,.form-field textarea{padding:8px 11px;border:1.5px solid var(--border);border-radius:6px;font-family:"DM Sans",sans-serif;font-size:13px}
...
<div class="topbar">
  ...
  <div class="topbar-right">
    ...
    ${isMultiTenant ? '<button class="btn-disconnect" onclick="location.href=\'/select-org\'">Switch Org</button>' : ''}
    ${isDemo ? '<button class="btn-disconnect" onclick="location.href=\'/connect\'">Connect Xero</button>' : '<button class="btn-disconnect" onclick="location.href=\'/disconnect\'">Disconnect</button>'}
  </div>
</div>
...
<div class="layout">
  <aside class="sidebar">
    <div class="balance-box">
      <div class="balance-lbl">Xero Balance</div>
  ...
  <main class="main">
    ...
    <div class="section active" id="section-dashboard">
      ...
      <div class="stats" id="dash-stats"><div class="loading"><div class="spinner"></div><br>Fetching your Xero data...</div></div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:18px">
        <div class="card"><div class="card-hdr"><span class="card-title">📥 Invoices Due — Next 30 Days</span></div><div class="card-body" id="dash-incoming"><div class="loading">Loading...</div></div></div>
        <div class="card"><div class="card-hdr"><span class="card-title">📤 Bills Due — Next 30 Days</span></div><div class="card-body" id="dash-outgoing"><div class="loading">Loading...</div></div></div>
      </div>
    </div>
      ...
      <div class="card">
        <div class="card-hdr">
          ...
          <div style="display:flex;align-items:center;gap:10px;font-size:12px">
            <span style="color:var(--light)">■ In</span>
            <span style="color:var(--orange)">■ Out</span>
            Alert threshold: <input type="number" id="threshold-input" value="10000" style="width:90px;padding:4px 8px;border:1.5px solid var(--border);border-radius:5px;font-size:12px" onchange="buildForecast()">
          </div>
        </div>
        <div class="card-body" style="overflow-x:auto">
          <div style="min-width:700px">
            ...
            <div id="forecast-rows"><div class="loading">Load dashboard first to build forecast</div></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="section" id="section-invoices">
      ...
      <div class="page-sub">Outstanding invoices from Xero — money coming in</div>
      ...
      <div class="card">
        ...
        <div class="tbl-wrap"><table>
          ...
          <tbody id="invoices-tbody"><tr><td colspan="6" class="loading">Loading from Xero...</td></tr></tbody>
        </table></div>
      </div>
    </div>
    ...
    <div class="section" id="section-bills">
      ...
      <div class="page-sub">Outstanding bills from Xero — money going out</div>
      <div id="bills-stats" class="stats"></div>
      <div class="card">
        ...
        <div class="tbl-wrap"><table>
          ...
          <tbody id="bills-tbody"><tr><td colspan="5" class="loading">Loading from Xero...</td></tr></tbody>
        </table></div>
      </div>
    </div>
    ...
    <div class="section" id="section-payroll">
      ...
      <div class="card">
        ...
        <div class="tbl-wrap"><table>
          ...
          <tbody id="payroll-tbody"><tr><td colspan="6" class="loading">Loading from Xero Payroll...</td></tr></tbody>
        </table></div>
      </div>
    </div>
    ...
    <div class="section" id="section-jobs">
      ...
      <div class="page-sub">Add upcoming jobs to see payment timing gaps in your forecast</div>
      <div class="alert alert-green">💡 <strong>Key feature:</strong> Enter upcoming jobs with payment terms. Runsheet maps when costs land vs when payment arrives — flagging cash gaps before they happen.</div>
...
<div class="modal-overlay" id="job-modal">
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div style="background:var(--sand);border-radius:8px;padding:14px;font-size:13px" id="job-preview-box">Enter details above to see margin and payment timing.</div>
    </div>
...
let appData = { invoices: [], bills: [], payRuns: [], jobs: JSON.parse(localStorage.getItem('rs_jobs') || '[]'), balance: 0, settings: JSON.parse(localStorage.getItem('rs_settings') || '{}') };

const fc = (n) => {
  if (n === null || n === undefined) return '—';
  const a = Math.abs(n);
  return (n < 0 ? '-$' : '$') + a.toLocaleString('en-AU', {minimumFractionDigits:0,maximumFractionDigits:0});
};
...
function nav(id) {
  ...
  document.querySelector(\`.section#section-\${id}\`).classList.add('active');
...
async function fetchData(url) {
  if (IS_DEMO) return getDemoData(url);
...
function getDemoData(url) {
  if (url === '/api/summary') return {
    tenantName: 'Creted Civil Pty Ltd (Demo)',
    totalReceivables: 231111.01,
    totalPayables: 46227.17,
    netPosition: 184883.84,
    invoices: [
      {client:'Metro Asphalt Pty Ltd',ref:'INV#20261085',amount:171833.75,due:'2026-04-28',status:'AUTHORISED'},
      {client:'Novacon Group',ref:'INV#20261084',amount:59277.26,due:'2026-04-23',status:'AUTHORISED'},
    ],
    bills: [
      {supplier:'Holcim Australia Pty Ltd',amount:10446.48,due:'2026-01-30',status:'OVERDUE'},
      {supplier:'Mesh & Bar Pty Ltd',amount:21801.97,due:'2026-04-15',status:'AUTHORISED'},
      {supplier:'Campbellfield Concrete',amount:4900.17,due:'2026-04-20',status:'AUTHORISED'},
    ]
  };
  if (url === '/api/payroll') return { payRuns: [
    ...
    {startDate:'2026-03-15',endDate:'2026-03-21',paymentDate:'2026-03-21',wages:18310,super:2105.65,payg:3662,totalNetPay:14648},
    {startDate:'2026-03-22',endDate:'2026-03-28',paymentDate:'2026-03-28',wages:18310,super:2105.65,payg:3662,totalNetPay:14648},
  ]};
  return {};
}
...
async function loadDashboard() {
  try {
    ...
    appData.balance = appData.settings.balance || 16875.81;
    document.getElementById('sidebar-balance').textContent = fc(appData.balance);
    document.getElementById('dash-sub').textContent = IS_DEMO ? 'Demo data — Creted Civil' : 'Live data from ' + data.tenantName;

    const now = new Date();
    const d30 = new Date(); d30.setDate(d30.getDate()+30);
    const in30 = appData.invoices.filter(i => new Date(i.due) <= d30 && new Date(i.due) >= now).reduce((s,i) => s+(i.amount||0), 0);
    const out30 = appData.bills.filter(b => new Date(b.due) <= d30 && new Date(b.due) >= now).reduce((s,b) => s+(b.amount||0), 0);
    const overdue = appData.invoices.filter(i => days(i.due) < 0).reduce((s,i) => s+(i.amount||0), 0);

    document.getElementById('dash-stats').innerHTML = \`
      <div class="stat"><div class="stat-lbl">Total Receivables</div><div class="stat-val">\${fc(data.totalReceivables)}</div><div class="stat-sub">unpaid invoices in Xero</div></div>
      <div class="stat \${overdue>0?'red':''}"><div class="stat-lbl">Overdue Invoices</div><div class="stat-val \${overdue>0?'neg':''}">\${fc(overdue)}</div><div class="stat-sub">\${overdue>0?'follow up immediately':'all current'}</div></div>
      <div class="stat amber"><div class="stat-lbl">Total Payables</div><div class="stat-val">\${fc(data.totalPayables)}</div><div class="stat-sub">bills in Xero</div></div>
    ...
    const alerts = [];
    if (overdue > 0) alerts.push(\`<div class="alert alert-amber">⚠️ <b>\${fc(overdue)}</b> in overdue invoices — chase immediately</div>\`);
    document.getElementById('dash-alerts').innerHTML = alerts.join('');

    const in30items = appData.invoices.filter(i => new Date(i.due) <= d30 && new Date(i.due) >= now).sort((a,b) => new Date(a.due)-new Date(b.due));
    document.getElementById('dash-incoming').innerHTML = in30items.length === 0
      ? '<div style="color:var(--muted);font-size:13px;text-align:center;padding:16px">No invoices due in next 30 days</div>'
      : in30items.slice(0,6).map(i => \`<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--sand);font-size:13px"><div><b>\${i.client||i.ref}</b><div style="font-size:11px;color:var(--muted)">Due \${i.due}</div></div><b style="color:#2D6A4F">\${fc(i.amount)}</b></div>\`).join('');

    const out30items = appData.bills.filter(b => new Date(b.due) <= d30 && new Date(b.due) >= now).sort((a,b) => new Date(a.due)-new Date(b.due));
    document.getElementById('dash-outgoing').innerHTML = out30items.length === 0
      ? '<div style="color:var(--muted);font-size:13px;text-align:center;padding:16px">No bills due in next 30 days</div>'
      : out30items.slice(0,6).map(b => \`<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--sand);font-size:13px"><div><b>\${b.supplier||'Supplier'}</b><div style="font-size:11px;color:var(--muted)">Due \${b.due}</div></div><b style="color:var(--danger)">\${fc(b.amount)}</b></div>\`).join('');

    toast('Data loaded from Xero ✓');
  } catch(e) {
    document.getElementById('dash-stats').innerHTML = '<div class="alert alert-red">⚠ Failed to load Xero data: ' + e.message + '</div>';
  }
}

function buildForecast() {
  const threshold = parseFloat(document.getElementById('threshold-input')?.value) || 10000;
  const invoices = appData.invoices;
  const bills = appData.bills;
  const jobs = appData.jobs;
  const weeklyOut = 22681; // wages + super + DD default
  let balance = appData.balance || 16875.81;
  ...
  for (let w = 0; w < 52; w++) {
    ...
    const inflows = invoices.filter(i => { const d=new Date(i.due); return d>=ws&&d<=we; }).reduce((s,i) => s+(i.amount||0), 0)
      + jobs.filter(j => j.paymentDate && new Date(j.paymentDate)>=ws && new Date(j.paymentDate)<=we).reduce((s,j) => s+(parseFloat(j.revenue)||0), 0);
    const outflows = bills.filter(b => { const d=new Date(b.due); return d>=ws&&d<=we; }).reduce((s,b) => s+(b.amount||0), 0) + weeklyOut;

    balance += inflows - outflows;
    const mo = ws.toLocaleDateString('en-AU',{month:'short',year:'2-digit'});
    const isDanger = balance < threshold;
    const isNeg = balance < 0;
    weeks.push({w:w+1, ws, we, inflows, outflows, balance, mo, isDanger, isNeg});
  }

  const maxFlow = Math.max(...weeks.map(w => Math.max(w.inflows, w.outflows)), 1);
  document.getElementById('forecast-rows').innerHTML = weeks.map(w => {
    const cls = w.isNeg?'neg':w.isDanger?'low':'ok';
    const rowCls = w.isNeg?'danger-row':w.isDanger?'amber-row':'';
    return \`<div class="week-row \${rowCls}">
      <div class="wk-lbl">Wk \${w.w}</div>
      <div class="wk-mo">\${w.mo}</div>
      <div class="wk-in">\${w.inflows>0?'+'+fc(w.inflows):'—'}</div>
      <div class="wk-out">\${w.outflows>0?'-'+fc(w.outflows):'—'}</div>
      <div class="wk-bal \${cls}">\${fc(w.balance)}</div>
      <div class="wk-bar">
...
async function loadInvoices() {
  document.getElementById('invoices-tbody').innerHTML = '<tr><td colspan="6" class="loading">Loading from Xero...</td></tr>';
  try {
    const data = IS_DEMO ? {invoices:appData.invoices} : await fetchData('/api/invoices');
    const invs = data.invoices || appData.invoices;
    const total = invs.reduce((s,i)=>s+(i.amount||0),0);
    const od = invs.filter(i=>days(i.due)<0).reduce((s,i)=>s+(i.amount||0),0);
    document.getElementById('inv-stats').innerHTML = \`
      <div class="stat"><div class="stat-lbl">Total Outstanding</div><div class="stat-val">\${fc(total)}</div></div>
      <div class="stat \${od>0?'red':''}"><div class="stat-lbl">Overdue</div><div class="stat-val \${od>0?'neg':''}">\${fc(od)}</div></div>
      <div class="stat"><div class="stat-lbl">Invoice Count</div><div class="stat-val">\${invs.length}</div></div>
    \`;
    document.getElementById('invoices-tbody').innerHTML = invs.length === 0
      ? '<tr><td colspan="6" style="text-align:center;padding:24px;color:var(--muted)">No outstanding invoices in Xero</td></tr>'
      : invs.map(i => {
          const d = days(i.due);
          return \`<tr><td><b>\${i.client||'—'}</b></td><td style="font-size:11px;color:var(--muted)">\${i.ref||'—'}</td>
            <td><b>\${fc(i.amount)}</b></td>
            <td style="color:\${d<0?'var(--danger)':d<=7?'var(--amber)':'inherit'}">\${i.due||'—'}</td>
            <td style="color:\${d<0?'var(--danger)':d<=7?'var(--amber)':'inherit'};font-weight:\${d<=7?700:400}">\${d<0?Math.abs(d)+' days OD':d+' days'}</td>
            <td><span class="badge \${d<0?'br':d<=7?'ba':'bg'}">\${d<0?'Overdue':i.status}</span></td></tr>\`;
        }).join('');
  } catch(e) {
    document.getElementById('invoices-tbody').innerHTML = \`<tr><td colspan="6" style="color:var(--danger);padding:16px">Error: \${e.message}</td></tr>\`;
  }
}

async function loadBills() {
  document.getElementById('bills-tbody').innerHTML = '<tr><td colspan="5" class="loading">Loading from Xero...</td></tr>';
  try {
    const data = IS_DEMO ? {bills:appData.bills} : await fetchData('/api/bills');
    const bls = data.bills || appData.bills;
    const total = bls.reduce((s,b)=>s+(b.amount||0),0);
    const od = bls.filter(b=>days(b.due)<0).reduce((s,b)=>s+(b.amount||0),0);
    document.getElementById('bills-stats').innerHTML = \`
      <div class="stat amber"><div class="stat-lbl">Total Outstanding</div><div class="stat-val">\${fc(total)}</div></div>
      <div class="stat \${od>0?'red':''}"><div class="stat-lbl">Overdue</div><div class="stat-val \${od>0?'neg':''}">\${fc(od)}</div></div>
      <div class="stat"><div class="stat-lbl">Bill Count</div><div class="stat-val">\${bls.length}</div></div>
    \`;
    document.getElementById('bills-tbody').innerHTML = bls.length === 0
      ? '<tr><td colspan="5" style="text-align:center;padding:24px;color:var(--muted)">No outstanding bills</td></tr>'
      : bls.map(b => {
          const d = days(b.due);
          return \`<tr><td><b>\${b.supplier||'—'}</b></td>
            <td><b>\${fc(b.amount)}</b></td>
            <td style="color:\${d<0?'var(--danger)':d<=7?'var(--amber)':'inherit'}">\${b.due||'—'}</td>
            <td style="color:\${d<0?'var(--danger)':d<=7?'var(--amber)':'inherit'};font-weight:\${d<=7?700:400}">\${d<0?Math.abs(d)+' days OD':d+' days'}</td>
            <td><span class="badge \${d<0?'br':d<=7?'ba':'bg'}">\${d<0?'Overdue':b.status}</span></td></tr>\`;
        }).join('');
  } catch(e) {
    document.getElementById('bills-tbody').innerHTML = \`<tr><td colspan="5" style="color:var(--danger);padding:16px">Error: \${e.message}</td></tr>\`;
  }
}

async function loadPayroll() {
  try {
    const data = await fetchData('/api/payroll');
    const runs = data.payRuns || [];
    document.getElementById('payroll-tbody').innerHTML = runs.length === 0
      ? '<tr><td colspan="6" style="text-align:center;padding:24px;color:var(--muted)">No pay runs found. Requires Xero Payroll subscription.</td></tr>'
      : runs.map(r => \`<tr>
          <td>\${r.startDate||'—'} – \${r.endDate||'—'}</td>
          <td>\${r.paymentDate||'—'}</td>
          <td><b>\${fc(r.wages||r.totalGross)}</b></td>
          <td style="color:var(--amber)">\${fc(r.super||r.totalSuperannuation)}</td>
          <td style="color:var(--orange)">\${fc(r.payg||r.totalPAYG)}</td>
          <td><b>\${fc(r.totalNetPay)}</b></td>
        </tr>\`).join('');
  } catch(e) {
    document.getElementById('payroll-tbody').innerHTML = \`<tr><td colspan="6" style="color:var(--danger);padding:16px">Error: \${e.message}</td></tr>\`;
  }
}
...
function renderJobs() {
  ...
  if (jobs.length === 0) {
    el.innerHTML = '<div style="text-align:center;padding:32px;color:var(--muted)">No jobs yet. Add upcoming jobs to see payment timing gaps in your forecast.</div>';
    return;
  }
  el.innerHTML = \`<div class="card"><div class="card-hdr"><span class="card-title">Job Pipeline — Payment Timing</span></div><div class="tbl-wrap"><table>
    ...
    <tbody>\${jobs.map((j,i) => {
      ...
      const costs = parseFloat(j.costs)||0;
      const margin = rev>0 ? ((rev-costs)/rev*100).toFixed(1) : 0;
      const gapDays = j.paymentDate && j.endDate ? Math.ceil((new Date(j.paymentDate)-new Date(j.endDate))/86400000) : null;
      const isLoss = parseFloat(margin)<0;
      return \`<tr>
        <td><b>\${j.name}</b></td><td>\${j.client}</td>
        <td style="color:#2D6A4F;font-weight:700">\${fc(rev)}</td>
        <td style="color:var(--danger)">\${fc(costs)}</td>
        <td style="font-weight:700;color:\${isLoss?'var(--danger)':parseFloat(margin)>=20?'#2D6A4F':'var(--amber)'}">\${margin}%</td>
        <td style="font-size:12px">\${j.paymentDate||'—'}</td>
        <td style="font-weight:700;color:\${gapDays&&gapDays>45?'var(--danger)':'inherit'}">\${gapDays?gapDays+' days':'—'}\${gapDays&&gapDays>45?' ⚠':''}</td>
        <td><span class="badge \${isLoss?'br':parseFloat(margin)>=20?'bg':'ba'}">\${isLoss?'Loss-Making':parseFloat(margin)>=20?'On Target':'Below Target'}</span></td>
        <td><button class="btn btn-outline" onclick="deleteJob(\${i})" style="font-size:11px;padding:4px 8px">✕</button></td>
      </tr>\`;
    }).join('')}</tbody></table></div></div>\`;
}

function previewJob() {
  const rev = parseFloat(document.getElementById('job-rev').value)||0;
  const costs = parseFloat(document.getElementById('job-costs').value)||0;
  const terms = document.getElementById('job-terms').value;
  const endDate = document.getElementById('job-end').value;
  if (!rev) { document.getElementById('job-preview-box').textContent = 'Enter details above.'; return; }
  const gp = rev-costs;
  ...
  const isLoss=parseFloat(margin)<0;
  document.getElementById('job-preview-box').innerHTML = \`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:13px">
      <div>Revenue: <b style="color:#2D6A4F">\${fc(rev)}</b></div>
      <div>Costs: <b style="color:var(--danger)">\${fc(costs)}</b></div>
      <div>Gross profit: <b>\${fc(gp)}</b></div>
      <div>Margin: <b style="color:\${isLoss?'var(--danger)':parseFloat(margin)>=20?'#2D6A4F':'var(--amber)'}">\${margin}%</b></div>
    ...
    \${isLoss?'<div class="alert alert-red" style="margin-top:8px">⛔ Loss-making job. Do not accept without renegotiating.</div>':''}
...
function saveJob() {
  ...
  if (!j.name||!j.revenue) { alert('Please enter job name and revenue'); return; }
  appData.jobs.push(j);
  localStorage.setItem('rs_jobs', JSON.stringify(appData.jobs));
  closeModal('job-modal'); renderJobs(); buildForecast(); toast('Job added ✓');
}

function deleteJob(i) {
  if (!confirm('Remove this job?')) return;
  appData.jobs.splice(i,1);
  localStorage.setItem('rs_jobs', JSON.stringify(appData.jobs));
  renderJobs(); toast('Job removed');
}
...
// Health check
app.get('/health', (req, res) => res.json({ status: 'ok', service: 'Runsheet', version: '1.0.0' }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`Runsheet running on port ${PORT}`));
